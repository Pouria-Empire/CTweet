...
int slre_match(const char *regexp, const char *s, int s_len,
               char caps_array[10][100000], int num_caps, int flags) {
    struct regex_info info;
    ...
    struct slre_cap caps[100000];
    info.caps = caps;

    DBG(("========================> [%s] [%.*s]\n", regexp, s_len, s));
    int result = foo(regexp, (int) strlen(regexp), s, s_len, &info);
    for (int i = 0; i < num_caps; ++i) {
        for (int j = 0; j < info.caps[i].len; ++j) {
            caps_array[i][j] = info.caps[i].ptr[j];
        }
    }
    return result;
}
